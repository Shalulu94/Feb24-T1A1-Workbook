...
## Q2: Define the features of the following technologies that are essential in terms of the development of the internet. Explain how each technology has contributed to the development of the internet:
 ...
 - **domains and DNS** - Web browsers operate and route information using IP Addresses, however IPv4 and IPv6 addresses are very long and hard to remember. The Domain Name System (DNS) converts IP Addresses into a simple domain name that humans can access and remember easier such as google.com or youtube.com. 

 ## Q3: Define the features of the following technologies that are essential in terms of the development of the internet:

- **TCP**: TCP, or Transmission Control Protocol, is a fundamental component of internet communication, boasting a suite of essential features to ensure secure, efficient data exchange between clients and servers. One key aspect is its secure connection establishment, achieved through a meticulous three-way handshake. This process involves synchronization and acknowledgment between both sides before any data transmission occurs, ensuring a robust foundation for communication. Furthermore, TCP incorporates mechanisms for flow control, which regulates the data transfer rate to prevent overwhelming either party and maintain a stable connection. Additionally, it supports full duplex communication, enabling simultaneous data transmission in both directions, thereby optimizing efficiency.

- **HTTP and HTTPS**: HTTP stands for Hypertext transfer protocol and is a method of data communication for the internet. It is an application layer protocol and mainly focuses on how information is presented to a user but is not secure when transmitting the data from point A to B. As such, it is not very secure and is susceptible to interception by malicious third parties. 
HTTPS is an extension of HTTP but adds an element of security by working in parallel with another protocol named Secure Sockets Later (SSL). By transporting data via the SSL layer, now commonly known as the Transport Layer Security (TLS), data now gets encrypted before being transmitted and cannot be intercepted. This sort of security is necessary to protect users from data breaches especially when inputting information such as passwords or credit card details. 
